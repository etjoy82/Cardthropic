...
pub(super) fn sanitize_selected_run(
    game: &KlondikeGame,
...
pub(super) fn foundation_count(game: &KlondikeGame) -> i64 {
    game.foundations()
        .iter()
...
pub(super) fn hidden_tableau_count(game: &KlondikeGame) -> i64 {
    game.tableau()
        .iter()
...
pub(super) fn face_up_tableau_count(game: &KlondikeGame) -> i64 {
    game.tableau()
        .iter()
        .flat_map(|pile| pile.iter())
        .filter(|card| card.face_up)
...
pub(super) fn empty_tableau_count(game: &KlondikeGame) -> i64 {
...
pub(super) fn auto_play_state_heuristic(game: &KlondikeGame) -> i64 {
    if game.is_won() {
...
pub(super) fn non_draw_move_count(game: &KlondikeGame) -> i64 {
    let mut count = 0_i64;
...
pub(super) fn restores_current_by_inverse_tableau_move(
    current: &KlondikeGame,
    next: &KlondikeGame,
    src: usize,
    dst: usize,
